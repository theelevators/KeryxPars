...
public class MappingSourceGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext context)
    {
        ...
        foreach (var candidateClass in classes)
        {
            ...
            if (hl7MessageAttribute != null || hl7ComplexTypeAttribute != null || hasFieldAttributes || hasComponentAttributes)
            {
                ...
                {
                    // Add the generated mapper class
                    var fileName = $"{classSymbol.Name}_Generated.g.cs";
                    context.AddSource(fileName, sourceCode);
                }

                // NEW: Generate convenience methods on the original class (if it's partial)
                if (classSymbol.DeclaringSyntaxReferences.Length > 0)
                {
                    var classDeclaration = classSymbol.DeclaringSyntaxReferences[0].GetSyntax() as ClassDeclarationSyntax;
                    if (classDeclaration != null)
                    {
                        bool isPartial = classDeclaration.Modifiers.Any(m => m.Text == "partial");
                        
                        if (isPartial)
                        {
                            // Generate the convenience methods
                            var convenienceCode = GenerateConvenienceMethods(classSymbol, isComplexType);
                            if (!string.IsNullOrEmpty(convenienceCode))
                            {
                                var convenienceFileName = $"{classSymbol.Name}_ConvenienceMethods.g.cs";
                                context.AddSource(convenienceFileName, convenienceCode);
                            }
                        }
                        else
                        {
                            // Report diagnostic: class should be partial for convenience methods
                            var diagnostic = Diagnostic.Create(
                                new DiagnosticDescriptor(
                                    "KERYX001",
                                    "Class should be partial",
                                    "Class '{0}' has HL7 mapping attributes but is not partial. Add 'partial' keyword to enable Parse/TryParse convenience methods.",
                                    "KeryxPars.Mapping",
    ...
    /// <summary>
    /// Generates convenience methods on the original class (Parse, TryParse, From).
    /// These methods wrap the Mapper class for better usability.
    /// </summary>
    private string GenerateConvenienceMethods(INamedTypeSymbol classSymbol, bool isComplexType)
    {
        var namespaceName = classSymbol.ContainingNamespace.ToDisplayString();
        var className = classSymbol.Name;
        var mapperName = $"{className}Mapper";
        ...
        sb.AppendLine();
        sb.AppendLine("using System;");
        sb.AppendLine("using KeryxPars.Core.Models;");
        sb.AppendLine("using KeryxPars.HL7.Mapping;");
        sb.AppendLine();
        sb.AppendLine($"namespace {namespaceName};");
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine($"/// Convenience methods for mapping HL7 messages to {className}.");
        sb.AppendLine("/// Generated by KeryxPars source generator.");
        sb.AppendLine("/// Uses Result&lt;T, E&gt; pattern instead of exceptions.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine($"public partial class {className}");
        sb.AppendLine("{");

        // Parse method (returns Result)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Parses an HL7 message into a {className} instance.");
        sb.AppendLine("    /// Returns Result instead of throwing exceptions.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"hl7Message\">The HL7 message as a read-only character span.</param>");
        sb.AppendLine($"    /// <returns>Result containing the mapped {className} or error.</returns>");
        sb.AppendLine($"    public static Result<{className}, HL7MappingError> Parse(ReadOnlySpan<char> hl7Message)");
        sb.AppendLine("    {");
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine($"            var value = {mapperName}.MapFromMessage(hl7Message);");
        sb.AppendLine($"            return Result<{className}, HL7MappingError>.Success(value);");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (HL7MappingException ex)");
        sb.AppendLine("        {");
        sb.AppendLine("            return new HL7MappingError(ex.Message, ex.FieldPath, ex.TargetType);");
        sb.AppendLine("        }");
        sb.AppendLine("        catch (Exception ex)");
        sb.AppendLine("        {");
        sb.AppendLine($"            return new HL7MappingError(ex.Message, \"N/A\", \"{className}\");");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();

        // Parse method (string overload)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Parses an HL7 message into a {className} instance.");
        sb.AppendLine("    /// Returns Result instead of throwing exceptions.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"hl7Message\">The HL7 message as a string.</param>");
        sb.AppendLine($"    /// <returns>Result containing the mapped {className} or error.</returns>");
        sb.AppendLine($"    public static Result<{className}, HL7MappingError> Parse(string hl7Message)");
        sb.AppendLine("    {");
        sb.AppendLine("        if (hl7Message == null)");
        sb.AppendLine("        {");
        sb.AppendLine($"            return new HL7MappingError(\"HL7 message cannot be null\", \"N/A\", \"{className}\");");
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        return Parse(hl7Message.AsSpan());");
        sb.AppendLine("    }");
        sb.AppendLine();

        // From method (alias for Parse - more fluent)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Creates a {className} instance from an HL7 message.");
        sb.AppendLine("    /// Alias for Parse() with a more fluent name.");
        sb.AppendLine("    /// Returns Result instead of throwing exceptions.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <param name=\"hl7Message\">The HL7 message as a read-only character span.</param>");
        sb.AppendLine($"    /// <returns>Result containing the mapped {className} or error.</returns>");
        sb.AppendLine($"    public static Result<{className}, HL7MappingError> From(ReadOnlySpan<char> hl7Message)");
        sb.AppendLine("    {");
        sb.AppendLine("        return Parse(hl7Message);");
        sb.AppendLine("    }");
        sb.AppendLine();

        // From method (string overload)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Creates a {className} instance from an HL7 message.");
        sb.AppendLine("    /// Alias for Parse() with a more fluent name.");
        sb.AppendLine("    /// Returns Result instead of throwing exceptions.");
        ...
        sb.AppendLine("    /// <param name=\"hl7Message\">The HL7 message as a string.</param>");
        sb.AppendLine($"    /// <returns>Result containing the mapped {className} or error.</returns>");
        sb.AppendLine($"    public static Result<{className}, HL7MappingError> From(string hl7Message)");
        sb.AppendLine("    {");
        sb.AppendLine("        return Parse(hl7Message);");
        sb.AppendLine("    }");
        sb.AppendLine();

        // TryParse method (legacy - marked obsolete)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Legacy TryParse method. Consider using Parse() which returns Result instead.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    [Obsolete(\"Use Parse() which returns Result<T, E> for better error handling\")]");
        sb.AppendLine($"    public static bool TryParse(ReadOnlySpan<char> hl7Message, out {className}? result)");
        sb.AppendLine("    {");
        sb.AppendLine("        var parseResult = Parse(hl7Message);");
        sb.AppendLine("        if (parseResult.IsSuccess)");
        sb.AppendLine("        {");
        sb.AppendLine("            result = parseResult.Value;");
        ...
        // TryParse method (string overload - legacy)
        sb.AppendLine("    /// <summary>");
        sb.AppendLine($"    /// Legacy TryParse method. Consider using Parse() which returns Result instead.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    [Obsolete(\"Use Parse() which returns Result<T, E> for better error handling\")]");
        sb.AppendLine($"    public static bool TryParse(string? hl7Message, out {className}? result)");
        sb.AppendLine("    {");
        sb.AppendLine("        if (hl7Message == null)");
        sb.AppendLine("        {");
        sb.AppendLine("            result = null;");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        return TryParse(hl7Message.AsSpan(), out result);");
        sb.AppendLine("    }");
